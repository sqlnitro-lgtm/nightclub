 ...
 * (réutilise data/voiceFollowStore.js, comme /followuser). Un seul membre
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (existing) {
      ...
      const embed = new EmbedBuilder().setColor(0x00b050).setDescription(`✅ <@${target.id}> n'est plus en laisse.`);
    ...
    const lockedNick = `${baseName} (${DOG_EMOJI} de ${ownerName})`.slice(0, 32);
    ...
    const embed = new EmbedBuilder()
      .setColor(0x999999)
      .setDescription(`${DOG_EMOJI} <@${target.id}> est maintenant en laisse — pseudo verrouillé, te suit automatiquement en vocal. Relance \`/dog\` sur lui pour le libérer.`);
